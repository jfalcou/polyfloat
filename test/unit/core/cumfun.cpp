...
struct mmax
{
  template<typename T> auto operator()(T a0) { return a0; }
  template<typename T> auto operator()(T a0, T a1) { return mpfr::max(a0, a1); }
  template<typename T> auto operator()(T a0, T a1, T a2) { return mpfr::max(mpfr::max(a0, a1), a2); }
};
...
<typename T>(T const& a0, T const& a1, T const& a2, T const& a3, T const& a4, T const& a5)
{
  ...
  using plf::cumfun;
  using plf::max;
  auto mcs = [](auto a0, auto a1) {
    return kumi::make_tuple(tts::mpfr_exec(mmax(), a0), tts::mpfr_exec(mmax(), a0, a1));
  };
  {
  ...
  T const& a0, T const& a1, T const& a2, T const& a3, T const& a4, T const& a5, T const& a6, T const& a7, T const& a8)
{
  ...
  using plf::cumfun;
  using plf::max;
  auto mcs = [](auto a0, auto a1, auto a2) {
    return kumi::make_tuple(tts::mpfr_exec(mmax(), a0), tts::mpfr_exec(mmax(), a0, a1),
