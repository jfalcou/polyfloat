...
namespace plf::_
{
  ...
  template<typename T> constexpr auto three_add_pedantic(T a, T b, T c) noexcept
  {
  ...
  template<typename T> constexpr auto three_add(T a, T b, T c) noexcept
  {
    auto [t0, t1] = eve::two_add(a, b);
    auto [hi, t2] = eve::two_add(t0, c);
    auto [md, lo] = eve::two_add(t2, t1);
  ...
  template<typename T> constexpr auto four_add(T a, T b, T c, T d) noexcept
  {
    auto [t0, t1] = eve::two_add(a, b);
