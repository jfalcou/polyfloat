...
namespace plf
{
  template<typename Options> struct sqrteps_t : eve::constant_callable<sqrteps_t, Options>
  {
    ...
    static POLYFLOAT_FORCEINLINE constexpr auto value(eve::as<T> const&, auto const&)
    {
      ...
      else if constexpr(plf::dimension_v<T> == 3)
      {
        ...
        else if constexpr(std::same_as<u_t, float>  ) return plf::_::from_triple<u_t>(0x1.6a09e6p-35, 0x1.9fcef4p-61, -0x1.b7ba68p-86);
  ...
  //!   namespace kyosu
  //!   {
  //!      template<kyosu::concepts::polyfloat_like T> constexpr auto sqrteps(T z) noexcept;
  //!   }
  //!   @endcode
  //!
  //!   **Parameters**
