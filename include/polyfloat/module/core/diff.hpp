...
namespace plf
{
  ...
  {
    template<std::size_t N, typename... Ts> struct result;
    ...
    POLYFLOAT_FORCEINLINE typename result<1, Tup>::type constexpr operator()(Tup const& t) const noexcept
    {
      return POLYFLOAT_CALL(t);
    }

    template<std::size_t N, eve::same_lanes_or_scalar_tuple Tup>
    POLYFLOAT_FORCEINLINE typename result<N, Tup>::type constexpr operator()(kumi::index_t<N>,
    ...
    POLYFLOAT_FORCEINLINE typename result<1, Ts...>::type constexpr operator()(Ts const&... ts) const noexcept
    {
    ...
    requires(eve::same_lanes_or_scalar<Ts...>)
    POLYFLOAT_FORCEINLINE typename result<N, Ts...>::type constexpr operator()(kumi::index_t<N>, Ts const&... ts) const
    {
...
namespace plf::_
{
  ...
  POLYFLOAT_NOINLINE constexpr auto diff_(POLYFLOAT_DELAY(), O const&, PT const& x) noexcept
  {
  ...
  POLYFLOAT_NOINLINE constexpr auto diff_(POLYFLOAT_DELAY(), O const&, kumi::index_t<N>, PT const& x) noexcept
  {
    ...
    else if constexpr (N == 0) return xx;
    else
    {
  ...
  POLYFLOAT_NOINLINE constexpr auto diff_(POLYFLOAT_DELAY(), O const&, Ts const&... xs) noexcept
  {
    return diff(kumi::make_tuple(xs...));
  }

  template<std::size_t N, plf::concepts::polyfloat_like... Ts, eve::callable_options O>
  POLYFLOAT_NOINLINE constexpr auto diff_(POLYFLOAT_DELAY(), O const&, kumi::index_t<N> in, Ts const&... xs) noexcept
  {
