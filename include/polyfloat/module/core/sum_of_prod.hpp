...
namespace plf
{
  ...
  template<typename Options> struct sum_of_prod_t : eve::strict_tuple_callable<sum_of_prod_t, Options, raw_option, pedantic_option>
  {
    template<typename... Ts> struct result : as_polyfloat_like<Ts...>
    {
    };

    template<concepts::polyfloat_like T0, concepts::polyfloat_like T1, concepts::polyfloat_like T2, concepts::polyfloat_like T3 >
    requires(eve::same_lanes_or_scalar<T0, T1, T2, T3>)
    EVE_FORCEINLINE typename result<T0, T1, T2, T3>::type constexpr operator()(T0 t0, T1 t1, T2 t2, T3 t3) const noexcept
    {
...
namespace plf::_
{
  ...
  POLYFLOAT_FORCEINLINE constexpr auto sum_of_prod_(POLYFLOAT_DELAY(), O const& o
                                                   , T1 const& a, T2 const& b
                                                   , T3 const& c, T4 const& d) noexcept
  {
