...
namespace plf
{
  ...
  template<typename Options> struct fnms_t : eve::strict_tuple_callable<fnms_t, Options, raw_option, pedantic_option>
  {
  ...
  //!   namespace polyfloat
  //!   {
  //!      template<polyfloat::concepts::polyfloat_like Z1, polyfloat_like Z2, polyfloat_like Z3> constexpr auto fnms(Z1 z1, Z2 z2, Z3 z3) noexcept;
  //!   }
  //!   @endcode
  //!
  //!   **Parameters**
  ...
  //!   **Return value**
  //!
  //!     Returns the fused negated-multiply substract of the arguments (-z1*z2-z3).
  //!
  //!  @groupheader{Example}
  //!
...
namespace plf::_
{
  ...
  POLYFLOAT_FORCEINLINE constexpr auto fnms_(POLYFLOAT_DELAY(), O const& , Z1 const& x, Z2 const& y, Z3 const& z) noexcept
  {
    return fma(minus(x), y, minus(z));
  }
}
