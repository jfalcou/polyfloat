# Contributing to POLYFLOAT

Thank you for your interest in POLYFLOAT. Bug reports, documentation fixes, a missing function and
whole new parts of the extended precision toolkit are all welcome.
...
merged pull requests. **You are welcome.** A student who wants to see how a number carries more
precision than its hardware type is exactly who we wrote this for, and nobody is going to hold a
...
issue](https://github.com/jfalcou/polyfloat/labels/good%20first%20issue) label lists work that is
...
```bash
cmake --preset gcc
cmake --build build/gcc --target unit.core.add.exe
ctest --preset gcc
```

One target per test file, and CMake globs the test directories, so a new `.cpp` needs no CMake edit.
The tests need MPFR and GMP, which is what a result is checked against, and POLYFLOAT pulls EVE:
build the one target you are working on rather than the suite.
...
- **the documentation of what you touched**, with the `polyfloat-doxygen` target still silent, since
  a warning here is a broken link on the website.
...
**A wrong result.** The fix and its test in the same commit.
...
instruction set it was compiled for, and the input values. A Godbolt link that reproduces it
replaces all three.

**Work on an open issue.** The [open issues](https://github.com/jfalcou/polyfloat/issues) are all
...
cmake --build build/gcc --target polyfloat-doxygen
```
